...
// TODO: check for any others missing from this list
var resourceAttributesMap = map[string]string{
...
func tagsToAttributes(tags []string, host string) (pcommon.Map, pcommon.Map, pcommon.Map) {
	...
	for _, tag := range tags {
		...
		if attr, ok := resourceAttributesMap[key]; ok {
			resourceAttrs.PutStr(attr, val)
		} else {
			dpAttrs.PutStr(translateDataDogKeyToOtel(key), val)
		}

		// TODO: Check which attributes to assign to scope attributes
	}
