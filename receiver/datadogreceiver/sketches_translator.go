...
func handleSketchesPayload(req *http.Request) (sp []gogen.SketchPayload_Sketch, err error) {
	buf := getBuffer()
	defer putBuffer(buf)
...
func translateSketches(sketches []gogen.SketchPayload_Sketch, mt *MetricsTranslator) pmetric.Metrics {
	bt := newBatcher()
	bt.Metrics = pmetric.NewMetrics()

	for _, sketch := range sketches {
		dimensions := parseSeriesProperties(sketch.Metric, "sketch", sketch.Tags, sketch.Host, mt.buildInfo.Version)
...
func sketchToDatapoint(sketch gogen.SketchPayload_Sketch_Dogsketch, dp pmetric.ExponentialHistogramDataPoint, attributes pcommon.Map) {
	dp.SetTimestamp(pcommon.Timestamp(sketch.Ts * 1_000_000_000)) // OTel uses nanoseconds, while Datadog uses seconds
...
func mapSketchBucketsToHistogramBuckets(sketchKeys []int32, sketchCounts []uint32) (map[int]uint64, map[int]uint64, uint64) {
	...
	for i := range sketchKeys {
		...
		histogramKey := sketchIndexToHistogramIndex(sketchKeys[i])
...
// sketchIndexToHistogramIndex converts a sketch bucket index into the closest equivalent
// exponential histogram index by calculating the lower bound of the sketch bucket from
// its index, and using that lower bound to compute an exponential histogram index that
// corresponds to an exponential histogram bucket which has a range covering that lower bound
func sketchIndexToHistogramIndex(sketchIdx int32) int {
	return histogramValueToIndex(sketchLowerBound(sketchIdx))
}
...
func histogramValueToIndex(value float64) int {
